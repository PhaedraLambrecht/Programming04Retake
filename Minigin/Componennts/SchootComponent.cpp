	...
	, m_SceneName{ "Demo" }
{
	...
	auto boundPlayerRespawn = std::bind(&SchootComponent::PlayerRespawn, this, std::placeholders::_1);
	PlayerEvent respawnEvent{ "PlayerRespawn", 0};
	EventManager::GetInstance().RegisterObserver(respawnEvent, boundPlayerRespawn);
...
void dae::SchootComponent::PlayerRespawn(const Event* e)
{
	...
	{
		if (event->playerIndex != unsigned(m_playerIndex))
			return;
